...
/*Get FS*/
#define GetFS(ucFlowStaus, pxFlowStatusBuf) (*(pxFlowStatusBuf) = (tFlowStatus)((ucFlowStaus) & 0x0Fu))
 ...
 static tN_Result CANTP_DoReceiveCF(tCanTpMsg * m_stMsgInfo, tCanTpWorkStatus *m_peNextStatus)
 {
     ...
     {	
 #ifdef EN_TP_DEBUG
         TP_DebugPrintf("Msg ID invalid in CF! RX ID = %X, RX ID = %X\n", 
     ...
     {
         TP_DebugPrintf("Msg SN invalid(0x%X), expect SN(0x%X)in CF!\n", m_stMsgInfo->aMsgBuf[0u], gs_stCanTPRxDataInfo.ucSN);
 ...
 static tN_Result CANTP_DoTransmitFC(tCanTpMsg * m_stMsgInfo, tCanTpWorkStatus *m_peNextStatus)
 {
     ...
     if(TRUE == g_stCANUdsNetLayerCfgInfo.pfNetTxMsg(txMsgId, 
