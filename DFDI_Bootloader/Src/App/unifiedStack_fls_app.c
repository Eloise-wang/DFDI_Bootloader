 ...
 */
 ...
 #define MAX_DATA_BUF_LEN (128u)  /*最大数据缓冲区长度*/
 
 /*用于解决取消定义 MAX_ONCE_CHECKSUM_SIZE 时出现的问题*/
...
#ifndef MAX_FLASH_DATA_LEN
#define MAX_FLASH_DATA_LEN PROGRAM_SIZE
#endif
...
do{\
    gs_stAppFlashStatus.isFlashErasedSuccessful = bIsFlashEraseSuccessful;\
}while(0u)

#define Flash_SetFlashProgramStatus(bIsFlashProgramSuccessful) \
do{\
    gs_stAppFlashStatus.isFlashProgramSuccessful = bIsFlashProgramSuccessful;\
}while(0u)
...
#define Flash_IsFlashEraseSuccessful() (gs_stAppFlashStatus.isFlashErasedSuccessful)
#define Flash_IsFlashProgramSuccessful() (gs_stAppFlashStatus.isFlashProgramSuccessful)
 #define Flash_IsFlashStructValid() (gs_stAppFlashStatus.isFlashStructValid)
 
 #define Flash_CreateAppStatusCrc(o_pCrc) do{\
     BSP_CRC_CalculateOnce((uint8 *)&gs_stAppFlashStatus, sizeof(gs_stAppFlashStatus) - 4u, o_pCrc);\
 }while(0u)
 ...
 */
 ...
 static uint8 Flash_Erase(boolean * o_pbIsOperateFinsh)
 {
    ...
    static tAPPType s_appType = APP_INVLID_TYPE;
     ...
     uint32 sectorNo = 0u;
     const uint32 maxEraseSectors = UDS_GetUDSS3WatermarkTimerMs() / BSP_Flash_GetEraseSectorMaxTimeMs();
                 ...
                 {				
                     ...
                     if(NULL_PTR != gs_stFlashDownloadInfo.stFlashOperateAPI.pfEraserSecotr)
                     {
                         ...
                         while(eraseSectorNoTmp)
                         {
                             ...
                             eraseFlashStartAddr += BSP_Flash_Get1SectorBytes();
                         }
 ...
 static uint8 Flash_Checksum(boolean * o_pbIsOperateFinsh)
 {
     ...
     {
         BSP_CRC_CalculateOnce((const uint8 *)gs_stFlashDownloadInfo.receivedDataStartAddr, gs_stFlashDownloadInfo.receivedDataLength, &xCountCrc);
     }
     /*Only flash driver is download, can do erase/write flash */
     else if(TRUE == Flash_IsFlashDriverDownload())	
     {
         BSP_CRC_Start(&xCountCrc);
         BSP_CRC_Append((const uint8 *)gs_stFlashDownloadInfo.receivedDataStartAddr, gs_stFlashDownloadInfo.receivedDataLength, &xCountCrc);
         BSP_CRC_End(&xCountCrc);
     ...
     if(TRUE == *o_pbIsOperateFinsh)
     {
         ...
         if(TRUE == Flash_IsReceivedCRCValid())	
         {
             if((TRUE == Flash_IsFlashDriverSoftwareData()))
             {
                 ...
                 if(TRUE != BSP_Flash_RegisterAPI(&gs_stFlashDownloadInfo.stFlashOperateAPI))
                 {
                     Flash_SetFlashDriverNotDonwload();
                 }
                 else
     ...
     *o_pbIsOperateFinsh = FALSE;
     ...
     switch(checksumStep)
     {
         ...
         case DO_CHECKING_CHECKSUM:
             ...
             BSP_CRC_Append((const uint8 *)startCalDataAddr, calCRCDataLen, &s_xCountCrc);
...
static void Flash_SaveCalculateCRCValue(const uint32 i_CRCValue)
{
 ...
 static boolean Flash_IsFlashDriverSoftwareData(void)
 {
     ...
     result = BSP_Flash_GetDriverInfo(&flashDriverStartAddr, &flashDriverEndAddr);
 ...
 static void Flash_ReadNewestAppInfoFromFlash(void)
 {
     ...
     result = BSP_Flash_GetAPPInfo(newestAppType, &appInfoStart, &appInfoBlocksize);
 ...
 static boolean Flash_SaveAppInfoData(const uint32 i_appInfoDataStartAddr)
 {
     ...
     res = BSP_Flash_ReadData(i_appInfoDataStartAddr, (sizeof(tAppFlashStatus)), &gs_stAppFlashStatus);
 #else
...
uint8 Flash_IsReadAppInfoFromFlashValid(void)
{
 ...
 uint8 Flash_WriteFlashAppInfo(void)
 {
    ...
    tAppFlashStatus *pAppStatusPtr = NULL_PTR;
     ...
     if(TRUE == result)
     {
         ...
         BSP_Flash_GetResetHandlerInfo(&bIsEnableWriteResetHandle, &resetHandleOffset, &resetHandleLength);
 
         /*update APP cnt*/
         if(TRUE == BSP_Flash_GetAPPInfo(newestAPPType, &newestAPPInfoStartAddr, &newestAPPInfoLen))
         {
         ...
         if(NULL_PTR != pAppStatusPtr)
         {
             ...
             result = BSP_Flash_WriteAPPInfoData(appInfoStartAddr,
                                                (uint8 *)pAppStatusPtr,
 ...
 static tAPPType Flash_DoCheckNewestAPPInfo(const tAppFlashStatus * i_pAppAInfo, const tAppFlashStatus *i_pAppBInfo)
 {
    ...
    boolean bIsAppAValid = FALSE;
     ...
     BSP_CRC_CalculateOnce((const uint8 *)i_pAppAInfo, sizeof(tAppFlashStatus) - 4u, &crc);
     if(crc == i_pAppAInfo->crc)
     {
         bIsAppAValid = TRUE;
     }
 
     BSP_CRC_CalculateOnce((const uint8 *)i_pAppBInfo, sizeof(tAppFlashStatus) - 4u, &crc);
 ...
 /*don't support APP B, so APP A is always newest.*/
     ...
     BSP_Flash_GetAPPInfo(APP_A_TYPE, &appInfoStartAddr_A, &appInfoBlockSize_A);
 
     BSP_Flash_GetAPPInfo(APP_B_TYPE, &appInfoStartAddr_B, &appInfoBlockSize_B);
 
 #ifdef EN_APP_INFO_DATA_IN_NONE_FLASH
     BSP_Flash_ReadData(appInfoStartAddr_A, appInfoBlockSize_A, (uint8 *)&appAInfo);
 
     BSP_Flash_ReadData(appInfoStartAddr_B, appInfoBlockSize_B, (uint8 *)&appBInfo);
 #else
     /*read APP A info*/
 ...
 boolean Flash_IsValidAddr(const tAPPType i_appType, const uint32 i_dataAddr, const uint32 i_dataLen)
 {
     ...
     res = BSP_Flash_GetConfigInfo(i_appType, &pBlockInfo, &item);
     if(TRUE == res)
     {
         while(index < item)
         {
 ...
 boolean Flash_IsFlashDriverData(const uint32 i_dataAddr, const uint32 i_dataLen)
 {
     ...
     res = BSP_Flash_GetDriverInfo(&flashDrvStartAddr, &flashDrvEndAddr);
     if(TRUE == res)
     {
