 ...
 */
 ...
 __attribute__((unused))  //已知该函数未被调用，让编译器不要报警告
 ...
 static void UDS_TriggerCoreSystemReset(void);
 ...
 static const uint8 gs_aGetVersion[] = {0x31u, 0x01, 0x03, 0xFFu};
 ...
 static void UDS_SecurityAccess(struct UDSServiceInfo* i_pstUDSServiceInfo, tUdsAppMsgInfo *m_pstPDUMsg)
 {
     ...
     switch(requestSubfunction)
     {
         case 0x01u :
             ...
             ret = BSP_UDS_ALG_GetRandom(AES_SEED_LEN, s_aSeedBuf);
 ...
 static void UDS_ResetECU(struct UDSServiceInfo* i_pstUDSServiceInfo, tUdsAppMsgInfo *m_pstPDUMsg)
 {
     ...
     {
         m_pstPDUMsg->pfUDSTxMsgServiceCallBack = NULL_PTR;
         UDS_SetNegativeErroCode(i_pstUDSServiceInfo->serNum, SFNS, m_pstPDUMsg);
     }
 }
 ...
 static void UDS_DoResetMCU(uint8 Txstatus)
 {
     ...
     {
        while(FLASH_IDLE != Flash_GetOperateFlashActiveJob())
         {
             Flash_OperateMainFunction();
             (void)BSP_WATCHDOG_Feed();
         }
 
         UDS_TriggerCoreSystemReset();
 ...
 __attribute__((unused))  // 知道这个函数未被使用，别报警告
 void UDS_SetIsRxUdsMsg(const uint8 i_setValue)
 {
     (void)i_setValue; //已知该参数未被使用，别报警告
 #ifdef EN_DELAY_TIME
     if(i_setValue)
     {
                                 ...
                                 const uint8 KeyLen)
 {
     ...
     BSP_UDS_ALG_Decrypt(i_pReceivedKey, KeyLen, aPlainText);
