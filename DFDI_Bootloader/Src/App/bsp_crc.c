 ...
 *     Brief: 【 AC7840 CRC校验BSP实现 - CRC-16/DNP软件查表】
 *       Note : 1. 适配芯片：AC78406
 *              2. 编码格式：UTF-8
 *              3. 编译环境：MDK-ARM / GCC
...
/*
 * CRC-16/DNP 查表
 * Poly: 0x3D65 (reflected poly = 0xA6BC)
 * Init: 0x0000
 * RefIn: true
 * RefOut: true
 * XorOut: 0xFFFF
...
bool BSP_CRC_Calculate(const uint8_t *pDataBuf, uint32_t dataLen, uint32_t *pCrc)
{
    ...
    {
        g_curCrc = (g_curCrc >> 8U) ^ g_dnpCrcTable[(g_curCrc ^ pDataBuf[index]) & 0xFFU];
    }
    *pCrc = g_curCrc;
