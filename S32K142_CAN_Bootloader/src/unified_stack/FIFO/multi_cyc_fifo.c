...
/*********************宏定义***************************/
#define STRUCT_LEN (20u) /*每个FIFO结构体占用空间*/
#define TOTAL_BYTES ((STRUCT_LEN) * (FIFO_NUM) + TOTAL_FIFO_BYTES) /*配置总字节数*/
/**********************************************************/

/*********************静态值定义**********************/
static unsigned char gs_ucFifo[TOTAL_BYTES] = {0};           /*total fifo len*/
static tFifoInfo *gs_pstListHeader = (tFifoInfo *)0u;        /*管理列表FIFO表头*/ 
static tLen gs_xCleanFifoLen = TOTAL_BYTES;                  /*FIFO的可用长度*/
...
**	Description		:	清空FIFO缓冲区，将读指针与写指针设置为相等
**	Input Parameter	:	i_xFifoId fifo id
**	Modify Parameter	:	none
