 ...
 *      Author: Eloise
 *     Project: DFDI_App
 *       Brief: 【 UDS配置层 - 适配AC7840，参考S32K142】
 *       Note : 1. 适配芯片：AC78406
 *              2. 编码格式：UTF-8
 *              3. 编译环境：MDK-ARM / GCC
 */
...
static void UDS_RoutineControl(struct UDSServiceInfo* i_pstUDSServiceInfo, tUdsAppMsgInfo *m_pstPDUMsg)
{
	...
	if(TRUE == UDS_IsGetVersion(m_pstPDUMsg))
	{
		...
		UDS_AppMemcopy(&aSWVersion[0u], sizeof(aSWVersion), &m_pstPDUMsg->aDataBuf[offset]);
		...
		UDS_AppMemcopy(&aHWVersion[0u], sizeof(aHWVersion), &m_pstPDUMsg->aDataBuf[offset]);
