...
/* 全局变量 */
static uint16 g_curCrc = 0U;

/*
 * 软件CRC查表计算
 * Poly: 0x3D65, RefIn/RefOut: true, XorOut: 0xFFFF
 */
static void CreatSoftwareCrc16(const uint8 *i_pDataBuf, const uint32 i_dataLen, uint16 *m_pCurCrc)
{
    uint32 index = 0u;

    for(index = 0u; index < i_dataLen; index++)
    {
        *m_pCurCrc = (uint16)(((*m_pCurCrc) >> 8) ^ g_dnpCrcTable[(((*m_pCurCrc) ^ i_pDataBuf[index]) & 0x00ff)]);
    }
}
...
bool BSP_CRC_Calculate(const uint8_t *pDataBuf, uint32_t dataLen, uint16_t *pCrc)
{
    ...
    CreatSoftwareCrc16(pDataBuf, dataLen, &g_curCrc);
    *pCrc = g_curCrc;
