 ...
 */
 ...
 /* 警告：所有选项长度（OPT_LENGTHs）必须至少为3，否则会引发错误 */
 #define MEMCPY_OPT_LENGTH   (0x00000009u)  /* 非对齐数据的9字节最优长度 */
 #define MEMMOVE_OPT_LENGTH  (0x00000008u)  /* 非对齐数据的8字节最优长度 */
 #define MEMSET_OPT_LENGTH   (0x00000007u)  /* 非对齐数据的7字节最优长度 */
 #define MEMCMP_OPT_LENGTH   (0x0000001Au)  /* 非对齐数据的26字节最优长度 */
 #define STRNCPY_OPT_LENGTH  (0x00000008u)  /* 非对齐数据的8字节最优长度 */
 ...
 /*
 * @brief 逐字节比较两块内存
 * @details 该函数是未针对长内存块对比做优化的内存比较函数版本，
 *          仅用于被内存比对函数调用，
 *          以完成长内存块中极短片段的对比操作。
 ...
 */
 ...
 /**
 * @brief 将数据从一个内存区域复制到另一个内存区域。
 * @details 该函数用于将数据从一个内存区域复制至另一个内存区域。
 *          内存区域**不得重叠**，且目标内存区域的大小必须大于或等于源内存区域的大小。
 *          无论源数据与目标数据采用何种对齐方式，本次复制操作均经过优化处理。
 ...
 */
 ...
 /**
 * @brief  计算32位有符号整数的绝对值（非负值）
 * @details If the number has minimal possible value (-鈭�9,223,372,036,854,775,808), result is
 *          undefined.
 * @param[in] s32Number Value to calculate with.
 * @return The absolute value of u32Number
 */
